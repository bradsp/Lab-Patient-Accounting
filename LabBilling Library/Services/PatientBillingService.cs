...
namespace LabBilling.Core.Services
{
    ...
    public sealed class PatientBillingService
    {
        ...
        public bool SaveCollectionRecord(BadDebt badDebt)
        {
        ...
        public string SendToCollections()
        {
            ...
            foreach (var result in results)
            {
                try
                {
                    var acc = unitOfWork.AccountRepository.GetByAccount(result.AccountNo, false);

                    result.DateSent = DateTime.Now;

                    unitOfWork.BadDebtRepository.Update(result, new[] { nameof(BadDebt.DateSent) });

                    unitOfWork.AccountNoteRepository.Add(acc.AccountNo, $"Account sent to collections. Write off amount {acc.Balance}");
