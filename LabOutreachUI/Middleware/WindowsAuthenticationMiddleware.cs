...
public class WindowsAuthenticationMiddleware
{
  private readonly RequestDelegate _next;
    ...
    {
        // Only process if user is Windows authenticated
        if (context.User?.Identity?.IsAuthenticated == true && context.User.Identity.AuthenticationType == "Negotiate")
        {
            var windowsUsername = context.User.Identity.Name;
            _logger.LogInformation("[WindowsAuthMiddleware] Processing Windows user: {Username}", windowsUsername);
     ...
     var identity = new ClaimsIdentity(new[]
{
  new System.Security.Claims.Claim("DbUserValidated", "false"),
      new System.Security.Claims.Claim("UnauthorizedReason", "DatabaseError")
        ...
        {
            _logger.LogWarning("[WindowsAuthMiddleware] No Windows authentication detected");
 }
