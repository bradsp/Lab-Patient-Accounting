...
-- =============================================
CREATE PROCEDURE [dbo].[usp_prg_pat_bill_actv_verify] 
...
(
	...
	= chrg.account
	WHERE dbo.chrg.credited = 0 AND cdm <> 'CBILL'
		AND pa.date_sent IS null
...
(
	SELECT TOP(100) PERCENT
		...
		'ACTV' AS [record_type],
		ROW_NUMBER() OVER (
			PARTITION BY E.account_id , chrg.cdm
			ORDER BY E.account_id, E.record_cnt_acct , chrg.cdm) 
		AS [record_cnt_acct],--E.record_cnt_acct ,
		E.patient_account_number AS [enctr_nbr],
		E.account_id AS [activity_id],
		'' AS [activity_type],
		--chrg.location,
		CONVERT(VARCHAR(10),E.acct_dates_of_service,101) AS [activity_date],--CONVERT(VARCHAR(10),chrg.service_date,101) AS [activity_date],
		'LABORTORY' AS [activity_description],
		'' AS [activity_code],
		'' AS [activity_amount]
		,chrg.qty AS [units]
		--SUM(chrg.qty*dbo.chrg.net_amt) OVER (PARTITION BY chrg.account,chrg.cdm) AS [chrg_amt]
		, chrg.cdm --dbo.amt.cpt4 AS [cpt_code]
		, cdm.descript AS [cdm_description]--cpt4.descript AS [cpt_description]
		, '' AS [drg_code]
		, '' AS [revenue_code]--amt.revcode AS [revenue_code]
		,'' AS [revenue_code_description]
		, '' AS [hcpcs_code]
		, '' AS [hcpcs_description]
		, '' AS [order_mgmt_activity_type]
		, chrg.calc_amt AS [activity_amount_due]--amt.amount AS [activity_amount_due]
		, E.acct_dates_of_service AS [activity_date_of_service]--CONVERT(VARCHAR(10), chrg.service_date,101) AS [activity_date_of_service]				
		, 0.00 AS [activity_patient_bal]
		, 0.00 AS [activity_ins_discount]
		, 'CHARGE' AS [activity_trans_type]
		, '' AS [activity_trans_sub_type]
		, chrg.calc_amt AS [activity_trans_amount]--chrg.qty*amt.amount AS [activity_trans_amount]
	...
	INNER JOIN cdm ON dbo.cdm.cdm = dbo.chrg.cdm
...
, @batch
FROM cteActv
--ORDER BY cteActv.statement_number, cteActv.activity_id,
--cteActv.record_cnt_acct
