...
-- =============================================
create PROCEDURE [dbo].[usp_prg_pat_bill_stmt_debug] 
...
SELECT dbo.patbill_acc.statement_number ,
		...
		WHERE dbo.patbill_acc.batch_id = @batch
			AND NULLIF(dbo.patbill_acc.date_sent,'') IS NULL	
...
, COALESCE(NULLIF(dbo.SplitCITY_ST_ZIP(pat.g_city_st,'C'),'')
	, NULLIF(dbo.SplitCITY_ST_ZIP(pat.city_st_zip,'C'),'')
	, NULLIF(pat.guar_city,'')
	, pat.pat_city)  AS [guarantor_city]
	
, COALESCE(NULLIF(dbo.SplitCITY_ST_ZIP(pat.g_city_st,'S'),'')
	, NULLIF(dbo.SplitCITY_ST_ZIP(pat.city_st_zip,'S'),'')
	, NULLIF(pat.guar_state,''), pat.pat_state
	) AS [guarantor_state]

, COALESCE(NULLIF(dbo.SplitCITY_ST_ZIP(pat.g_city_st,'Z'),'')
	,NULLIF(dbo.SplitCITY_ST_ZIP(pat.city_st_zip,'Z'),'')
	,NULLIF(dbo.pat.guar_zip,''), pat.pat_zip
	) AS [guarantor_zip]
