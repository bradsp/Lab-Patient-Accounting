...
-- =============================================
CREATE PROCEDURE [dbo].[usp_prg_pat_bill_stmt_small_balance] 
...
dbo.GetDaysSinceLastPayment(cte.account) AS [Days Since Last Paymnet]
, (
	CAST(DATEPART(YEAR,GETDATE())AS FLOAT )*10000000)
	+((CAST(DATEPART(MONTH,GETDATE())AS FLOAT )*100000)
	+ROW_NUMBER() OVER (ORDER BY cte.id)
  ) AS [statement_number]
--, (DATEPART(MONTH,GETDATE())*100000)+ROW_NUMBER() OVER (ORDER BY cte.id) AS [statement_number1]
...
cte.guarantor_zip ,
cte.guarantor_name ,
cte.amount_due,--) OVER (PARTITION BY cte.id) AS [amount_due] ,
cte.date_due ,
cte.balance_forward,--) OVER (PARTITION BY cte.id) AS [balance_forward]  ,
cte.aging_bucket_current,--) OVER (PARTITION BY cte.id) AS [aging_bucket_current] ,
cte.[30_Day],--) OVER (PARTITION BY cte.id) AS [30_Day],
cte.[60_Day],--) OVER (PARTITION BY cte.id) AS [60_Day],
cte.[90_Day],--) OVER (PARTITION BY cte.id) AS [90_Day] ,
cte.statement_total_amount,--) OVER (PARTITION BY cte.id) AS [ statement_total_amount] ,
--cte.insurance_billed_amount ,
cte.balance_forward_amount,--)  OVER (PARTITION BY cte.id) AS [balance_forward_amount] ,
...
--cte.insurance_pending ,
		cte.unpaid_balance,--) OVER (PARTITION BY cte.id) AS [unpaid_balance],
		cte.patient_balance,--) OVER (PARTITION BY cte.id) AS [patient_balance] ,
		cte.total_pat_paid_since_last_stmt,--) OVER (PARTITION BY cte.id) AS [total_pat_paid_since_last_stmt] ,
...
--		cte.includes_est_pat_liab ,
		cte.total_charge_amount--) OVER (PARTITION BY cte.id) AS [total_charge_amount] 
...
FROM cte
--WHERE rn  = 1
ORDER BY dbo.GetDaysSinceLastPayment(cte.account) 
END
