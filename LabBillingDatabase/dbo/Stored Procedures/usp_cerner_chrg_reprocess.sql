...
-- =============================================
CREATE PROCEDURE [dbo].[usp_cerner_chrg_reprocess]
...
BEGIN
	...
	IF ((NULLIF(SUSER_NAME(),'WTHMC\bpowers')) IS NOT NULL)
	BEGIN
		PRINT N'Procedure being run by invalid user.'
		RETURN 99;
	END	
...
--	.value('.','int') AS [ureka]
	...
	SELECT @xml = (select x FROM #temp3)
...
--			, 'varchar(25)') AS [USER NUMBER]				
	...
	, RIGHT(concat('usp_cerner_chrg_reprocess - ',@src_app),50)  AS [mod_prg]
