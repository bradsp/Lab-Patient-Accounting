...
-- =============================================
CREATE PROCEDURE usp_prg_Charges_With_Payments 
...
AS
(
	SELECT ROW_NUMBER() OVER (PARTITION BY chk.account  ORDER BY chk.account) AS [rn],
	chk.account, COALESCE(ins_code,source,ins_code,'WO'+dbo.chk.write_off_code,dbo.chk.comment) AS [source], SUM(amt_paid) AS [paid], 
		SUM(contractual) AS [contractual],
		SUM(write_off) AS [write off]
		FROM dbo.chk
		INNER JOIN cteChrg ON cteChrg.account = dbo.chk.account
		WHERE chk.account <> @client
		GROUP BY chk.account, COALESCE(ins_code,dbo.chk.source,ins_code,'WO'+dbo.chk.write_off_code,dbo.chk.comment)
)
