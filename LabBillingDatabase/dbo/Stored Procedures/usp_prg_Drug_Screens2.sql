...
-- =============================================
CREATE PROCEDURE [dbo].[usp_prg_Drug_Screens2] 
...
BEGIN
	...
	-- 1. set the charge credited
--	update chrg
--	set credited = 1
--	where chrg_num = @OrigChrgNum

	-- 2. add the reversal record 
--	INSERT INTO chrg
...
--	WHERE     (chrg_1.chrg_num = @OrigChrgNum)
	...
	-- get the charge number for the new inserted record in order to add the amount record
	SET @chrgRowguid = '0451e481-4f3a-4669-b3fa-f0cb2f3492f6'
...
--	, bill_method)
	SELECT     
	 @ChrgRowguid, 0, account, 'NEW', service_date, getdate(), '5362566', 1, (select sum(mprice) from cpt4 where cdm = '5362566')
	, (select sum(zprice) from cpt4 where cdm = '5362566'), 'G0143 Conversion'
	,  getdate(), right(suser_sname(),50), right('usp_prg_Drug_Screen '+convert(varchar(10),getdate(),112),50)
