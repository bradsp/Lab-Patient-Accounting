...
-- =============================================
CREATE PROCEDURE [dbo].[usp_prg_UpdateGCodeBilling] 
...
--1. set the end date according to the end of the month processing
	...
	TRUNCATE TABLE dbo.Temp_GCodeBilling

	INSERT INTO dbo.Temp_GCodeBilling
			(
				colClient ,colAcc ,colOrigChrgNum ,
				colCDM ,colCPT ,colQty ,colChrgAmt ,
				colDOS ,colDateEntered ,colPrice ,colError
			)
	SELECT colClient ,
			...
			, colPrice
			,colError 
			FROM dbo.GetGCodeBillingCharges(@endDate) g
...
--	LEFT outer JOIN dbo.Temp_GCodeBilling t ON REPLACE(REPLACE(colAcc,'c','j'),'d','j') = acc.account 
...
--        )
--		SELECT  REPLACE(REPLACE(acc.account,'C','J'),'D','J') , pat_name ,  'JPG' ,  'Y' ,
...
--		FROM acc	
--		WHERE account IN (SELECT DISTINCT colAcc FROM dbo.Temp_GCodeBilling t
--						LEFT OUTER JOIN acc ON acc.account = 
--							REPLACE(REPLACE(t.colAcc,'c','j'),'d','j')
--						WHERE acc.account IS NULL)
----SELECT 'Acc: ' + CONVERT(VARCHAR(2),@@ROWCOUNT )
--		INSERT INTO dbo.acc_merges
--				( account ,dup_acc , mod_date ,mod_prg ,mod_user ,mod_host)
--		SELECT acc.account,REPLACE(REPLACE(acc.account,'C','J'),'D','J'),				
--				  GETDATE() 
...
----IF (NOT EXISTS(
----	SELECT account FROM acc 
----	INNER JOIN dbo.Temp_GCodeBilling t ON REPLACE(REPLACE(colAcc,'c','j'),'d','j') = acc.account
----	))
----	BEGIN
----		INSERT INTO dbo.acc
----        ( account, pat_name, cl_mnem , fin_code,
...
----				, RIGHT (HOST_NAME(),50)  AS [mod_host],bill_priority ,guarantorID 
----		FROM acc	
----		WHERE account IN (SELECT DISTINCT colAcc FROM dbo.Temp_GCodeBilling)
----
----		INSERT INTO dbo.acc_merges
----				( account ,dup_acc , mod_date ,mod_prg ,mod_user ,mod_host)
----		SELECT acc.account,REPLACE(REPLACE(acc.account,'C','J'),'D','J'),				
...
--LEFT outer JOIN dbo.Temp_GCodeBilling t 
--	ON REPLACE(REPLACE(colAcc,'c','j'),'d','j') = pat.account
...
--        )
--		SELECT  REPLACE(REPLACE(pat.account,'C','J'),'D','J') ,pat_full_name, pat.ssn ,
--			pat_addr1 ,pat_addr2 ,city_st_zip, pat_city,pat_state, pat_zip
--			,dob_yyyy ,sex ,relation ,
--			guarantor ,guar_addr ,g_city_st , guar_city, guar_state,guar_zip
--			, pat_marital , pat_race, pat_phone
...
--		FROM pat
--		WHERE account IN (SELECT DISTINCT colAcc FROM dbo.Temp_GCodeBilling t
--		LEFT OUTER JOIN pat ON pat.account = REPLACE(REPLACE(t.colAcc,'c','j'),'d','j')
--		WHERE pat.account IS NULL )
--		
--		End
--
...
--LEFT outer JOIN dbo.Temp_GCodeBilling t ON REPLACE(REPLACE(colAcc,'c','j'),'d','j') = ins.account
...
--          mod_host
--        )
--		SELECT  deleted ,REPLACE(REPLACE(ins.account,'C','J'),'D','J') ,ins_a_b_c ,
--			holder_nme ,holder_dob ,holder_sex ,holder_addr ,holder_city_st_zip ,
--			plan_nme ,plan_addr1 ,plan_addr2 ,p_city_st ,
--			policy_num ,cert_ssn ,
--			grp_nme ,grp_num ,
...
--		FROM ins
--		WHERE account IN (SELECT DISTINCT colAcc FROM dbo.Temp_GCodeBilling t
--		LEFT OUTER JOIN dbo.ins ON dbo.ins.account = REPLACE(REPLACE(t.colAcc,'c','j'),'d','j')
...
--	, 'MOVED TO ['+REPLACE(REPLACE(account,'C','J'),'D','J')+']' as [comment]
--	--, NULL AS [invoice]
--	, GETDATE() AS [mod_date], RIGHT(SUSER_SNAME(),50) AS [mod_user]
...
--	)
--	SELECT dbo.Temp_GCodeBilling.colNewChrgNum AS [chrg_num]
--	, cpt4, type, amount
--	, GETDATE() AS [mod_date], RIGHT(SUSER_SNAME(),50) AS [mod_user]
--	, COALESCE(RIGHT(OBJECT_NAME(@@PROCID),50),'SQL PROC ' + CONVERT(VARCHAR(10),GETDATE(),112)) AS [mod_prg]
...
--		bill_type, bill_method, pointer_set	
--	FROM amt 
--	CROSS APPLY dbo.Temp_GCodeBilling 
--	WHERE amt.chrg_num = dbo.Temp_GCodeBilling.colOrigChrgNum
--	
--	
...
--		, mod_date, mod_user, mod_prg, mod_host
--		, net_amt, fin_type, fin_code, bill_method
--	)
--	SELECT gb.rowguid
--	, REPLACE(REPLACE(colAcc,'C','J'),'D','J')	
...
--	FROM dbo.temp_GCodeBilling gb
--	CROSS APPLY dbo.GetPrice('JPG','CLIENT',gb.colCDM,null) u
--	
--	UPDATE dbo.Temp_GCodeBilling
--	SET colNewChrgNum = c.chrg_num
--	, colPrice = c.net_amt
--	FROM dbo.Temp_GCodeBilling t
--	CROSS APPLY MCLLIVE.dbo.chrg c 
--	where c.rowguid = t.rowguid AND c.account = REPLACE(REPLACE(t.colAcc,'C','J'),'D','J')	
--
...
--FROM MCLLIVE.dbo.temp_GCodeBilling t
...
----	SELECT * FROM dbo.acc
--	WHERE account 
--		IN (SELECT REPLACE(REPLACE(colAcc,'c','J'),'d','J') FROM dbo.Temp_GCodeBilling)
--	AND status = 'PAID_OUT'
--
---- do this last because if nothing else works we should roll back the changes and 
---- want to have the charge available to retry.
--	UPDATE dbo.chrg
--	SET bill_method = 'JPG2'
--		,credited =  1
--		--, comment = 'MOVED TO ['+REPLACE(REPLACE(account,'C','J'),'D','J')+']'
--	FROM dbo.Temp_GCodeBilling
--	WHERE chrg_num IN (SELECT colOrigChrgNum FROM dbo.Temp_GCodeBilling)
--
