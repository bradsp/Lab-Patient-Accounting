...
-- =============================================
CREATE PROCEDURE [dbo].[usp_prg_UpdateGlobalBilling] 
...
--1. set the end date according to the end of the month processing
	...
	--SET @startDate = CAST('06/01/2020 00:00' AS DATETIME) -- date program started
...
where dbo.ins.account is null))
	...
	INSERT INTO MCLLIVE.dbo.chrg
	(
		rowguid,account, service_date, hist_date, cdm, qty
		, inp_price
		, retail, comment
		, mod_date, mod_user, mod_prg, mod_host
	...
	, colCDM, colQty
	, u.colInpPrice
	, colQty*colChrgAmt, 'MOVED FROM ['+colAcc+']' AS [comment]
	, GETDATE() AS [mod_date], RIGHT(SUSER_SNAME(),50) AS [mod_user]
