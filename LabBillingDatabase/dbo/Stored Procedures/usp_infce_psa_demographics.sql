...
-- Description:	
-- =============================================
CREATE PROCEDURE [dbo].[usp_infce_psa_demographics] 
...
--	SET @endDate = '02/02/2014';
	...
	SELECT a.account, 
		(SELECT
			...
			acc.meditech_account AS 'PID/PID.18',
			...
			fin.fin_code AS 'PV1/PV1.20/PV1.20.1',
			fin.res_party AS 'PV1/PV1.20/PV1.20.2',
			...
			REPLACE(ia.relation,CHAR(0),'') as 'IN1/IN1.17',
			...
			REPLACE(ib.relation,CHAR(0),'') as 'IN1/IN1.17',
			...
			REPLACE(ic.relation,CHAR(0),'') as 'IN1/IN1.17',
			...
			CAST((SELECT
				...
				CASE WHEN COALESCE(icd.link,'') <> '' THEN icd.icd_indicator ELSE NULL END as 'DG1.3/DG1.3.3',
		...
		LEFT OUTER JOIN fin ON fin.fin_code = acc.fin_code
	...
	FROM acc a
	where a.account IN (SELECT * FROM dbo.ufn_jpg_activity_idx(CONVERT(nvarchar(10),getdate()-1,101))) --RUN FOR ACTIVITY >= YESTERDAY
--		a.account IN (select chrg.account from chrg JOIN amt ON chrg.chrg_num = amt.chrg_num
--						where chrg.service_date >= @beginDate and service_date < @endDate
--						AND chrg.cdm IN (SELECT cdm.cdm FROM cdm JOIN cpt4 ON dbo.cpt4.cdm = dbo.cdm.cdm
--											WHERE cpt4.type = 'PC' OR cpt4.cpt4 IN ('83020','84165','84166','86077','86334','86335')
--											GROUP BY cdm.cdm))
END
