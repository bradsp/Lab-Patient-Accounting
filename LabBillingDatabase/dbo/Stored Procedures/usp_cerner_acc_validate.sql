...
-- =============================================
CREATE PROCEDURE usp_cerner_acc_validate
...
-----------------
SELECT @msgID, @xml AS [xmlData], @xmlDemo AS [xmlDemo]

DECLARE @chrgFin VARCHAR(10)
DECLARE @insFin VARCHAR(10)
...
SELECT  COALESCE(
		...
		NULLIF(dbo.GetMappingValue('FIN_CODE','CERNER','d'),'K')
		,'Y')

select dbo.GetMappingValue('FIN_CODE','CERNER',NULLIF(@chrgFin,'H')) AS mapped
SELECT dbo.GetMappingValue('FIN_CODE','CERNER',@insFin) as [ins fin]

,dbo.GetMappingValue('FIN_CODE','CERNER',@chrgFin) AS [chrg fin]

, COALESCE(NULLIF(dbo.GetMappingValue('FIN_CODE','CERNER',NULLIF(@chrgFin,'H')),'K'),@insFin,'Y') AS [coal fin]
return
IF (@xml IS NULL)
BEGIN
...
SELECT @acc AS [Acc],@transDate AS [tdate], @fin AS [fin code]
return
